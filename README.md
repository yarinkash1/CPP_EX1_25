...
1. פונקציה `bfs` – מקבלת גרף וקודקוד התחלה ומחזירה גרף שהוא עץ שהתקבל מסריקת BFS. בנוסף (ראו את האלגוריתם בספר של קורמן או בתומרים של אלגוריתמים 1) הפונקציע תעדכן מערך שיכיל את המרחקים הקצרים ביותר לכל קודקוד.
...
7. לצורך מימוש אלגוריתמים אלו תצטרכו לממש באופן בסיסי מבני נתונים נוספים: תור, תור עדיפויות ו UNION FIND. 
